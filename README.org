...
The user option ~agent-log-directory~ specifies the root directory of your Claude Code configuration.  The package reads =history.jsonl= in this directory and looks for session files under the =projects/= subdirectory.  The default value is =~/.claude=.  You would change this only if you have configured Claude Code to use a non-standard location.  This option applies to the Claude Code backend; each backend has its own directory configuration.
